...
function hasLocalnet(rpcUrl) {
    return rpcUrl.includes("127.0.0.1") || rpcUrl.includes("localhost");
}
function toIx(encoded, initSignerPubkeys = []) {
...
test("web parity localnet: init_profile and create_open+start_single succeed", async (t) => {
    ...
    const startSingleIx = await program
        ...
        .instruction();
    const gameTx = new Transaction().add(toIx(createOpenIx, [matchState.publicKey.toBase58()]), toIx(startSingleIx));
